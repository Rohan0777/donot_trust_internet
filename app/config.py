...
# 여론이 아닌 채널. 이해관계자(브로커·거래소)·기계·기업 원문·재배포망이 만든 글이라
# 가중치가 0이며, 가중치를 켜는 것이 사용자 선택지에 없다. 그러므로 채점도 하지 않는다.
# 실측(2026-08-24 미채점 잔량): vendor 4,380 + pr 693 = 5,073건이 지수에 1도 기여하지
# 않는 문서에 LLM 호출의 13%를 쓰고 있었다.
#
# [community 를 여기 넣지 않는 이유]
# community 의 가중치 0은 '성격'이 아니라 '커버리지 불균형'에서 온 임시 조치다
# (DEFAULT_TIER_WEIGHTS 주석 참조). 균형 잡힌 소스를 확보하면 켜야 하는데, 그때
# 라벨이 없으면 과거 구간을 다시 채점해야 한다 — 소급 채점이 가능한 자원이 아니다
# (raw_documents 본문은 채점 성공 시 파기된다).
NON_OPINION_CHANNELS = ("vendor", "algo", "pr", "aggregator")
