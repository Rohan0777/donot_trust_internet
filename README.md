# 인터넷을 믿지 마세요 (trust-no-internet)

뉴스·커뮤니티의 긍정/부정 신호를 지수화하고, 그 신호를 그대로 따라 매매했을 때
자산이 어떻게 변하는지를 보여주는 공개 웹 서비스.

> 이 프로젝트의 결론은 "감성으로 돈을 벌 수 있다"가 아니다.
> **비용을 반영하고 나면 계좌가 어떻게 녹는지**를 보여주는 것 자체가 콘텐츠다.

## 청사진 v0.3

### 데이터 플로우

```
[L0 수집]  google_news_rss(백필·다국어) · naver_news 스크래핑(최근 구간)
           naver_search_api(일간 증분 전용) · naver_board(종토방, 커버리지 제한)
           fourchan(/biz/, 백필 불가 — 매일 실행 필수)
             ↓ body_hash 계산 후 raw_documents에 임시 저장
[L1 정규화] 제목 정규화 → title_hash(완전일치) → simhash(근사) → rapidfuzz 확정
             ↓ dup_group_id 부여, 대표만 is_canonical=1
[L2 채점]  대표글 중 여론 채널만 LLM/FinBERT → {positive, negative, neutral, irrelevant}
             (vendor·algo·pr·aggregator 는 가중치 0이라 채점 자체를 건너뛴다)
             ↓ 성공 시 raw_documents 행 삭제 (본문 파기)
[L3 영구]  documents — 날짜/매체/등급/라벨/제목/확산도
             ↓
[L4 집계]  sentiment_daily (code × date × media_id)
             ↓  tier 롤업은 sentiment_daily_tier 뷰로
[L5 서빙]  FastAPI — 가중치는 조회 시점 파라미터
[L6 UI]    일/주/월 가격차트 + 매체별·등급별 감성 + 자산곡선
```

### 확정된 설계 결정 14가지

| # | 결정 | 근거 |
|---|---|---|
| 1 | 시각은 **UTC로 저장**, KST 변환은 조회 계층 | 구 프로젝트는 소스별 naive/aware 혼재로 +9h 오차 |
| 2 | 본문은 `raw_documents`에 임시 저장, **파기 전 `body_hash`** | 파기 후에는 중복판정 능력을 영구히 잃음 |
| 3 | 집계는 **media 단위**, tier 롤업은 뷰 | 매체별 차트 요구 충족. tier 대비 행 수 21배(실측) |
| 4 | 중복은 삭제하지 않고 **`is_canonical=0` 강등** | 재배포 횟수(확산도)가 독립 피처 |
| 5 | 라벨에 **`irrelevant` 4번째 클래스** | "카카오톡 잡담"이 neutral로 흡수되어 지수를 0으로 끌어내림 |
| 6 | **`prompt_version`/`label_model` 기록** | 프롬프트 교체 시 과거/신규 라벨 혼재 → 인공적 regime shift |
| 7 | 신호 윈도우 `[D-1 15:30, D 08:50]` → **D일 시가 체결** | "익일 시가"로 미루면 하루를 이중으로 낭비 |
| 8 | 거래비용은 **`fee_schedule` 테이블** | 거래세율은 연도별 변경. 하드코딩 금지 |
| 9 | `coverage`에 **`partial`/`empty` 상태 분리** | 부분 수집을 완료로 찍으면 그 날짜가 영구히 반쪽으로 고정 |
| 10 | 가중치 0 채널은 **채점도 하지 않는다** (`NON_OPINION_CHANNELS`) | 지수에 1도 기여하지 않는 문서에 LLM 호출의 31%를 쓰고 있었다 |
| 11 | 화면에 **채점률(`labeled_ratio`)을 함께 싣는다** | 라벨률 2%와 99%가 같은 표에 서면 채점 진척도를 시장 특성으로 오독한다 |
| 12 | 검색어는 `aliases_json`이 아니라 **`queries_json`** | 별칭은 `normalize_title()`의 입력이라, 늘리면 과거 문서와 중복 판정이 어긋난다 |
| 13 | 같은 실행 안에서 `coverage` 상태는 **나쁜 쪽이 이긴다** | 질의 5개 중 마지막이 `completed`로 덮으면 상한에 걸린 사실이 사라진다 |
| 14 | 수집 미완결일은 **보정하지 않고 표시만** 한다 | 잘린 참값을 모르므로 추정해 채우면 없는 숫자를 만드는 것이다 |

### 감성지수 산식

```
온도지수 T(t) = (pos−neg)/(pos+neg) × n/(n+k)        # 방향 × 표본신뢰도, k=10
관여도   E(t) = z( log(1+doc_cnt) )                   # 방향과 무관한 "관심 온도"
확산도   D(t) = Σ spread_sum                          # 재배포 횟수 = 파급력
백테스트 신호 = Σ w·(pos − neg)                        # 표시용과 달리 순건수를 쓴다
```

`w`는 `media.tier` → `DEFAULT_TIER_WEIGHTS` 매핑이며 **조회 시점 파라미터**다.
슬라이더를 움직여도 재집계하지 않는다.

### 채점률을 화면에 싣는 이유 (실측)

감성지수는 라벨이 붙은 문서만으로 만들어지는데, 채점은 발행일 역순으로 진행된다.
따라서 채점이 밀린 시장의 지수는 "그 시장의 여론"이 아니라 **최근 구간에 쏠린
일부 문서**를 재고 있다. 2026-08-24 채점 전후 실측:

| 시장 | 채점 전 | 채점 후 | 시차 상관 (채점 전 → 후) |
|---|---:|---:|---|
| BTC | 6% | 98% | lag+2 r=+0.644 (변화 없음) |
| ETH | 2% | 99% | lag+3 r=+0.577 → lag+2 r=+0.650 |
| SPX | 2% | 97% | **분석 불가** → lag+1 r=+0.585 |
| NASDAQ | 47% | 98% | lag−4 r=−0.450 → lag−4 r=−0.437 |

NASDAQ이 이 장치가 필요한 이유를 그대로 보여준다. 채점률 47%에서 `lag=−4,
r=−0.450`, 즉 **"여론 선행 + 역행"** 이라는 이 사이트의 가설을 정확히 뒷받침하는
가장 탐나는 신호가 나왔다. 절반의 문서로 나온 숫자였으므로 그대로 결론으로
내걸었다면 근거 없는 주장이 됐을 것이다. 전수 채점 후에도 −0.437(차분 후 −0.303)로
남았으니 이번에는 진짜였지만, **그 사실은 채점을 끝낸 뒤에야 알 수 있었다.**

그래서 `MIN_LABELED_RATIO = 0.60` 미만인 시장은 결과를 내되 화면에서 격리한다 —
표에 ⚠ 표시, 시차 곡선은 점선, 그리고 **"유의한 시장 N곳" 요약 집계에서 제외**한다.
마지막 항목이 핵심이다. 미완 시장을 요약에 넣으면 그 문장 자체가 채점 진척도의
함수가 되어, 채점을 더 돌릴 때마다 사이트의 결론이 바뀐다.

### 축소항 k가 필요한 이유 (실측)

| 하루 글 수 | 일수 | \|극성\| 평균 | ±1.0 극단값 |
|---|---:|---:|---:|
| 1~5건 | 129 | 0.725 | **62.8%** |
| 6~20건 | 132 | 0.575 | 22.0% |
| 21~100건 | 52 | 0.570 | 11.5% |
| 100건+ | 21 | 0.391 | **0%** |

축소항이 없으면 **차트에서 가장 크게 튀는 날이 근거가 가장 빈약한 날**이 된다.
긍정 1건·부정 0건이면 +1.0으로, 긍정 300·부정 200인 날(+0.2)보다 5배 낙관적으로
보인다. k=10에서 극단값이 사라지고 지수 범위(최대 0.89)는 유지된다.

### 글 수를 지수에 넣어야 하는가 — 넣어야 한다 (실측)

지수와 익일 시가수익률의 상관 (이상치 제거 후):

| 산식 | SK하이닉스 | 카카오 |
|---|---:|---:|
| 극성만 | −0.118 | +0.021 |
| **순건수 (글 수 반영)** | **−0.247** | **−0.130** |

글 수를 반영하면 신호가 약 2배가 된다. 부호가 음수인 것은 **여론과 주가가
역행**한다는 뜻으로, 이 사이트의 논지와 일치한다. 하이닉스는 |z|=2.6으로
유의하고 카카오는 |z|=1.8로 경계선이다 — 종목마다 다르다는 것 자체가 관찰 결과다.

## 현재 상태

| 항목 | 값 |
|---|---|
| documents | **129,416** |
| 라벨 보유 | **108,130** (v3 프롬프트) |
| 수집 구간 | 2025-07-29 ~ 2026-08-25 |
| 매체 등급 미분류 | 16.5% |
| 중복·도배 강등 | 16,355 |
| DB | 165.6 MB (서빙 스냅샷 8.3 MB) |

출처: 구글뉴스 90,395 / 종토방 25,900 / 네이버뉴스 8,058 / 검색API 3,987 /
카페 941 / 4chan 135

미분류가 12.2% -> 17.0%로 오른 것은 퇴보가 아니다. 다국어 수집으로 해외 매체가
대량 유입돼 분모가 커졌고, 동시에 브로커·자동생성 리서치·보도자료 배포망을
news 분모 밖으로 빼면서 기준 자체가 엄격해졌다.

### 채널별 라벨 분포

| 채널 | 긍정 | 중립 | 부정 | 무관 | 중립비율 |
|---|---:|---:|---:|---:|---:|
| news | 25,365 | 42,365 | 16,237 | 33,542 | 50.5% |
| community | 3,353 | 9,231 | 10,031 | 4,952 | **40.8%** |
| cafe | 21 | 556 | 48 | 511 | 89.0% |
| *vendor* | 107 | 245 | 51 | 226 | 60.8% |
| *aggregator* | 111 | 184 | 93 | 174 | 47.4% |
| *algo* | 14 | 75 | 9 | 75 | 76.5% |
| *pr* | 14 | 19 | 0 | 18 | 57.6% |

구 파이프라인의 커뮤니티 중립 비율은 **99.1%** 였다(인코딩 파손 + 반어법 미처리).

기울임 4개는 가중치 0으로 고정된 채널이고, 위 수치는 규칙을 넣기 전에 이미 채점된
분량이다. **지금은 채점 대상에서 제외되므로 더 늘지 않는다** (`NON_OPINION_CHANNELS`).

### 알려진 부채

- **종토방 커버리지 불균형** — 대형주는 하루 2만 건이 올라오는데 게시판
  페이지네이션이 1,001p에서 끊긴다(실측 SK하이닉스 19,279건 = 1.5일치).
  뉴스는 180일을 덮으므로 켜두면 그 이틀이 6개월 곡선을 지배한다.
  → `community` 기본 가중치를 **0**으로 두고, UI에 사유를 노출한다.
  균형 잡힌 커뮤니티 소스를 확보하기 전까지 유지.
- **매체 등급 미분류 17.0%** — 롱테일. 등록도메인 단위로 접어 매칭하는데도
  남는 해외 소형 매체가 대부분이다.
- **해외 지수는 수집분의 절반 이상이 여론이 아니다** — 미채점 잔량 기준
  NASDAQ은 제외 채널(aggregator 1,662 + algo 801 + pr 335 + vendor 610)이
  채점 대상(2,803)에 육박했다. 한국 종목(000660은 제외분 0)과 수집 구성이
  근본적으로 다르므로, 시장 간 비교에서 "시장마다 다르다"를 읽을 때 감안해야 한다.
- **영어 지수 질의는 무관 비율이 높다** — 실측 SPX 57.7% / NASDAQ 53.6%
  (한국 종목 news는 그보다 낮다). 질의어가 넓어 `irrelevant`가 걸러내고 있다는
  뜻이지만, 질의 자체를 좁히면 표본이 늘어날 여지가 있다.
- **수집 완전성(`coverage.partial`)이 종목마다 크게 다르다.** `partial`은 "그 날짜를
  끝까지 훑지 못했다"는 뜻이고, 응답 상한에 걸린 날이 그렇게 남는다. 실측:

  | code / source | partial 비중 |
  |---|---:|
  | 035720 / naver_news | **100%** (43일) |
  | NASDAQ / google_rss | 71% (84일) |
  | KOSDAQ / google_rss | 64% (84일) |
  | ETH / google_rss | 55% (84일) |
  | BTC / google_rss | 39% (176일) |
  | KOSPI / google_rss | 32% (84일) |
  | 005930 / google_rss | 28% (366일) |

  **이것이 지수 자체를 오염시킨다.** 백테스트 신호는 극성이 아니라 순건수
  `Σ w·(pos − neg)`이고 관여도는 `E = z(log(1+doc_cnt))`인데, 상한에 걸린 날은
  건수가 100에서 잘린다. "여론이 폭발한 날"과 "상한에 걸린 날"이 구분되지 않고,
  편향이 무작위가 아니라 **신호가 가장 강한 날에 집중된다.**
  `healthcheck`가 상시 감시한다. 우회 수단은 별칭 확장(질의당 100건이 따로 나온다)뿐이다.
- **`v.daum.net` 1,402건** — 포털 집계 링크라 원 매체를 알 수 없다.
- **지수·코인의 질의 확장이 남아 있다.** `partial` 비중이 가장 높은 것은 개별주가
  아니라 NASDAQ(71%)·KOSDAQ(64%)·ETH(55%)다. 질의를 늘리면 상한을 우회할 수 있지만,
  중립적인 질의어를 고르기가 개별주보다 훨씬 어렵다(위 `queries` 절 참조). [확인 필요]

## 사고 기록: 인코딩 파손 (재발 방지)

`finance.naver.com`은 페이지마다 인코딩이 다르다.

```
/item/news_news.naver  → charset=EUC-KR
/item/board.naver      → charset=UTF-8    ← 종목토론방
```

구 코드가 두 페이지 모두에 `encoding="euc-kr"`를 하드코딩해서 종토방 제목
**28,114/28,286건(99.4%)** 이 U+FFFD로 파손됐다. 복원 불가. 그 상태로 LLM 채점까지
나가 **99.1%가 neutral**로 찍혔고(대조군 naver_news 36.9%), 그 결과 "커뮤니티
인간지표" 차트 전체가 노이즈였다.

방어 장치 2개를 넣었다 — `http_utils._resolve_encoding()`(헤더 charset 우선)과
`assert_decoded()`(U+FFFD 비율 초과 시 예외). **인코딩을 호출부에서 지정하지 말 것.**

## 웹 서비스 실행

```powershell
python -m scripts.serve            # http://127.0.0.1:8000
python -m scripts.serve --reload   # 개발 모드
```

기본 바인딩은 `127.0.0.1`이다. 외부 공개는 `--host 0.0.0.0`을 명시할 때만 일어난다.

### 화면 구성

| Zone | 내용 |
|---|---|
| Header | 종목 · 일별/주간/월간 · 기간(데이터 구간/3M/6M/1Y/전체) · 데이터 보유 구간 |
| Zone 1 | 주가 + 감성지수 오버레이. 종합 / 등급별 / 매체별(칩 선택 최대 6개). 데이터 없는 구간은 회색 음영, **수집 미완결일은 하단 앰버 리본** |
| Zone 2 | ① 정방향·역배팅 ② 보유수량내·무한공매도 ③ **비용 반영(기본 ON)** ④ 등급별 가중치 6개 |
| Zone 3 | **3선 분해** — 순손익 / 주식 평가액 / 현금 잔고 + Buy&Hold. 차트 위에 실질 수익률 대표 숫자 |

### 백테스트 계산

```
raw(D)   = Σ_tier w_tier × (pos − neg)        # 중립은 애초에 들어가지 않는다
desired  = ±raw                                # 역배팅이면 부호 반전
delta    = max(desired, −position) | desired   # 보유수량내 | 무한공매도
체결      = open(D)                            # 신호는 D−1 15:30 ~ D 08:50 확정
보유비용  = |position|×close×borrow/365        # 연율 bps → 일할. 공매도 차입
           + |cash|×financing/365              # 현금 부족 이자 (fee_schedule)
순손익    = cash + position × close(D)         # 0원 시작, 모든 모드에서 정의됨
```

**시드머니가 없으므로 수익률의 분모는 `최대 소요자본`이다.** 누적 투입금을 분모로 쓰면
매도가 쌓여 음수가 될 때 손실이 +수익률로 표시되고, 0을 지날 때 발산한다.
최대 소요자본 = max(현금 잔고 최저점의 절댓값, 공매도 익스포저 최대값).

보유비용(carry)은 수수료와 별개로 포지션이 밤새 유지되는 동안 붙는 돈이다 — 공매도는
주식 차입비용이고, 시드머니 0 구조에서는 매수 순간 현금이 음수가 되므로 **정방향
매수에도** 금융비용이 붙는다. 양의 현금에는 이자를 주지 않는다(주면 논지에 유리하게
숫자를 만지는 꼴이 된다).

적용 요율(2026-08-25): **금융 연 9.38%**(금융투자협회 공시 기준 국내 10대 증권사
신용융자 비대면 최장구간 평균), **차입 연 2.8%**(금감원 자료 — 공매도 잔액 상위
10종목 평균 대여수수료). 차입 실요율은 개인 신용대주 이자가 공시되지 않아 알 수
없으므로 2.8%는 **하한 추정**이다 — 실제로는 더 높고, 따라서 표의 수치는 여전히
비용 쪽으로 관대하다. 요율 변경:

```sql
UPDATE fee_schedule SET borrow_bps_annual=?, financing_bps_annual=?
 WHERE effective_from=(SELECT MAX(effective_from) FROM fee_schedule);
```

### 검증된 동작 (116거래일 · 6개월 · 커뮤니티 가중치 0)

**카카오 (035720)**

| 방향 | 포지션 | 비용 | 수익률 | 순손익 |
|---|---|:---:|---:|---:|
| 정방향 | 보유수량내 | ON | −26.13% | −5,661,779원 |
| 정방향 | 무한공매도 | ON | −13.80% | −2,620,089원 |
| 역배팅 | 보유수량내 | ON | −9.87% | −3,455,309원 |
| 역배팅 | 무한공매도 | ON | **+13.58%** | +2,297,365원 |

정방향(여론 추종)은 네 조합 중 어느 쪽도 이익을 내지 못했다. 사이트의 논지가
실제 데이터에서 나온다.

비용 토글은 항상 결과를 악화시킨다 — 22거래일 표본에서는 `정방향/무한공매도`가
비용 OFF에서 **+0.62%**, ON에서 **−0.04%** 로 부호가 뒤집혔다.

## 관측 대상 (entities)

개별 종목뿐 아니라 지수·코인·채권을 같은 테이블에 담는다. `documents.code`가
그대로 FK이므로 코인은 `code='BTC'`가 된다.

```powershell
python -m scripts.seed_entities          # docs/entities.csv 적용
python -m scripts.seed_entities --list   # 등록 상태 조회
```

| kind | 대상 | calendar | 가격 소스 |
|---|---|---|---|
| index | KOSPI, KOSDAQ, NASDAQ, SPX | krx / us | KS11 · KQ11 · IXIC · US500 |
| crypto | BTC, ETH, ALTCOIN | **crypto (24x7, 휴장일 없음)** | BTC/KRW · ETH/KRW · **합성 지수** |
| bond | KTB, UST | krx / us | **148070 · IEF (ETF 가격, 금리 아님)** |
| commodity | GOLD | us | GC=F |
| equity | 000660, 035720, 005930 | krx | pykrx |

`calendar`가 단순 라벨이 아니다 — 코인은 휴장일이 없어 주말 감성을 다음 거래일로
넘기는 롤포워드 규칙이 주식과 다르게 적용돼야 한다.

### `aliases`와 `queries`는 다른 것이다

**`aliases_json`은 검색어가 아니다.** 세 군데에서 동시에 쓰인다:

| 쓰이는 곳 | 역할 |
|---|---|
| `entity_queries()` | 검색 질의어 (`queries_json`이 없을 때만) |
| `_is_offtopic()` | 관련성 판정 — 제외어를 지운 뒤에도 남아야 한다 |
| **`normalize_title()`** | **제목에서 지워지는 토큰 → `title_hash`·`simhash`의 입력** |

세 번째가 함정이다. 별칭을 추가하면 **새로 들어오는 문서만 다르게 정규화되어 이미
쌓인 문서와 중복 판정이 어긋난다.** 그래서 검색 범위를 넓힐 때는 `queries_json`을
쓴다 — 비어 있으면 `aliases_json`으로 대체되므로 기존 엔티티는 그대로 동작한다.

**질의는 각각 독립이고, 응답 상한도 질의마다 따로 걸린다.** 시장은 표기가 여럿이고
(`코스피`/`KOSPI`/`코스피지수`) 각각 다른 기사 집합을 반환하므로 하나만 쓰면 절반을
놓친다. 대형주는 이유가 하나 더 있다 — Google News RSS는 질의당 100건이 상한이고
**하루 단위로 쪼개도 뚫리지 않으므로, 질의를 늘리는 것이 상한을 우회하는 유일한
수단이다.** 중복은 URL UNIQUE가 막으므로 겹쳐 돌려도 안전하다. 한국어 질의는 한국
로케일, 영어는 미국 로케일로 자동 분기하고 엇갈린 조합은 요청하지 않는다.

실측(005930, 1년 · 2025-08-25 ~ 2026-08-25):

| 질의 | 창 | 상한 도달 | 비율 | 신규 |
|---|---:|---:|---:|---:|
| `삼성전자` | 253 | 99 | **39%** | 1,295 |
| `삼성전자 주가` | 149 | 26 | 17% | 3,062 |
| `삼성전자 실적` | 61 | 13 | 21% | 906 |
| `삼성전자 반도체` | 191 | 52 | 27% | 2,636 |
| `삼성전자 HBM` | 74 | 6 | **8%** | 730 |
| 합계 | 728 | 196 | 27% | **8,629** |

**질의 확장은 상한을 완화하지만 제거하지 못한다.** 확장 질의도 8~27%가 상한에
걸린다(기본 질의는 39%). 대신 확보량이 다르다 — 기본 질의 1,295건 대 확장 4질의
7,334건이다. 3일 창 표본에서는 확장 질의가 한 번도 상한에 걸리지 않아 "상한을
피한다"고 오판했었다. 표본이 짧으면 뉴스가 몰린 날이 안 들어온다.

**질의어는 반드시 주제어여야 한다.** `삼성전자 하락` 같은 방향성 단어를 넣으면 그
질의가 끌어온 기사가 통째로 한쪽 논조로 기울어 **감성지수 자체가 편향된다.** 이
프로젝트가 재는 것이 그 감성이므로 치명적이다. 같은 이유로 지수(`KOSPI`/`NASDAQ` 등)의
질의 확장은 보류했다 — `코스피 마감`·`코스피 시황` 같은 중립어조차 "그날 지수가 어떻게
됐는지 서술하는 기사"를 늘려 아래 [주의 2]의 `lag>0` 항등식을 강화한다. [확인 필요]

## 수집기 실행 (분석/서빙과 분리된 독립 프로세스)

```powershell
python -m scripts.collect status                        # 원장/실행이력 조회
python -m scripts.collect master                        # KOSPI 종목마스터
python -m scripts.collect prices 000660 --years 3
python -m scripts.collect news   000660 --days 30           # 네이버 금융 종목뉴스
python -m scripts.collect board  000660 --days 30           # 종목토론방
python -m scripts.collect gnews  KOSPI --start 2026-01-01 --end 2026-07-01
python -m scripts.collect biz                               # 4chan /biz/ (crypto 전체)
```

⚠️ **`biz`는 매일 실행해야 한다.** 4chan 공식 API는 살아있는 카탈로그만 제공하고
만료된 스레드는 영구 삭제된다(과거 스레드 번호는 전부 404). **백필 경로가 존재하지
않으므로 놓친 날은 되돌릴 수 없다.** 다른 소스는 며칠 놓쳐도 소급 수집이 되지만
이것만은 예외다.

### 매일 자동 수집

```powershell
python -m scripts.daily                  # 수집 + 채점 + 집계 한 번에
python -m scripts.daily --catchup 14     # 놓친 날짜까지 소급

# 스케줄러 등록 (매일 09:10 / 21:10)
powershell -ExecutionPolicy Bypass -File scripts\install_schedule.ps1
```

`coverage`에서 결손 날짜를 찾아 소급하므로 노트북이 며칠 꺼져 있어도 복구된다.
**4chan만 예외** — 소급 경로가 없어 놓친 날은 영구 손실이다. 하루 2회 도는 이유가
이것이다.

`daily`는 자체 잠금(`data/daily.lock`)으로 중복 실행을 막는다 — 스케줄러와 수동
실행이 겹치면 media INSERT 경쟁으로 라운드가 죽는다(2026-08-25 실측). 이미 돌고
있으면 즉시 종료(exit 3). 6시간 넘은 락은 비정상 종료 잔존물로 보고 회수한다.

### 수집 / 서빙 분리

웹은 수집 DB를 아예 열지 않는다. 화면에 필요한 것은 사전집계와 가격뿐이므로
그것만 담은 스냅샷을 만들어 배포한다.

```powershell
python -m scripts.export_snapshot          # data/serve.db 생성
python -m scripts.export_snapshot --verify # 검증만

$env:TNI_DB = "D:\dev	rust-no-internet\data\serve.db"
python -m scripts.serve
```

| | |
|---|---|
| 수집 DB | 131.3 MB |
| **서빙 DB** | **6.2 MB (4.7%)** |
| 포함 | entities · media · prices · sentiment_daily · fee_schedule · **label_coverage · date_coverage** |
| **제외** | **documents · raw_documents · coverage · pipeline_runs** |

원문·URL·작성자가 웹 호스트에 아예 존재하지 않으므로 유출면이 줄고, 수집이 몇
시간 돌아도 사이트 응답에 영향이 없다. 노트북이 꺼져 있어도 사이트는 산다.

`label_coverage`만 예외적으로 `documents`를 접어서 만든다. "이 지수가 몇 %의 문서로
만들어졌는가"는 원본 없이 계산할 수 없는데, 그 수치가 없으면 사이트가 채점 미완
시장을 완료된 시장과 같은 표에 나란히 세우기 때문이다. 코드·건수만 담고 문서는
담지 않는다.

### 후처리 / 유지보수

```powershell
python -m scripts.map_media                # 매체 등급 적용 + 한글명·도메인 병합
python -m scripts.map_media --report       # 미분류 비중 조회
python -m scripts.recover_press            # URL에서 매체 복원 (네트워크 불필요)
python -m scripts.dedup --all              # 근사중복 판정 + 도배 억제
python -m scripts.score 035720 --dry-run   # 채점 비용 추정 (제외분도 함께 표시)
python -m scripts.score 035720 --limit 500 # LLM 채점 (OPENAI_API_KEY 필요)
python -m scripts.score BTC --include-non-opinion  # 가중치 0 채널까지 (기본은 제외)
python -m scripts.refresh                  # 사전집계 재계산 (healthcheck가 요구할 때)
python -m scripts.healthcheck              # 파이프라인 상태 점검
```

`healthcheck`는 조용히 잘못되기 쉬운 것만 골라 검사한다 — 시각 정규화, 인코딩
파손, 프롬프트 버전 혼재, 매체 미분류 비중, **사전집계와 문서의 정합성**, 가격
결손, 최근 수집일, 미채점 잔량. 각 항목은 "이게 깨지면 어떤 숫자가 거짓이 되는가"를
기준으로 골랐다. 종료 코드는 실패 2 / 경고 1 / 정상 0이라 스케줄러에 물릴 수 있다.

장시간 백필은 백그라운드로 떼어놓는다. 진행상황은 로그 파일과 `pipeline_runs`
테이블 양쪽에 남으므로, 콘솔을 닫아도 `status`로 확인할 수 있다.

```powershell
# 백그라운드 시작
Start-Process python -ArgumentList "-m","scripts.collect","board","000660","--days","180","--max-pages","20000","--log","logs\board_000660.log" -WorkingDirectory "D:\dev\trust-no-internet" -WindowStyle Hidden

# 진행 확인
Get-Content logs\board_000660.log -Tail 20 -Wait
python -m scripts.collect status
```

중단해도 안전하다. 페이지 단위로 커밋하고 `coverage`에 `partial`+`last_cursor`를
남기므로, 같은 명령을 다시 실행하면 이미 받은 URL은 건너뛴다.

## 데이터 소스별 제약 (구 프로젝트에서 실제로 겪은 것)

- **네이버 검색 오픈API는 백필에 쓸 수 없다.** 쿼리당 1000건 상한(`start<=1000`),
  날짜 범위 지정 불가(`sort=date`만). `days_back`을 키워도 같은 최신 1000건이
  돌아온다. 구 코드가 이걸로 1년치 루프를 돌려 신규 0건을 얻었다. **일간 증분 전용.**
- **Google News RSS는 쿼리당 100건 상한이고, 창 밖 기사를 섞어 100건을 채운다.**
  따라서 "응답이 100건" ≠ "잘렸다". 절단 판정은 반드시 *요청한 날짜 창 안에 든*
  건수로 해야 한다 — 응답 건수로 판정하면 이미 완전 수집한 날짜가 영영 `partial`에
  갇힌다. 실측: 카카오 1일 창에서 응답 100건 중 창내 15건인 날이 흔하다.
- **Google News의 `link`는 리다이렉트라 원문 URL을 알 수 없다.** 본문은 수집하지
  않고 제목만 쓴다. 대신 `<source url>`이 매체명·도메인을 정확히 준다.
- **종목명 모호성은 수집 단계에서 걸러야 한다.** "카카오" 질의는 카카오뱅크·
  카카오페이 기사를 함께 끌어온다(실측 6개월 백필에서 제외 대상이 전체의 40% 수준).
  `stocks.exclude_json`의 제외어를 지운 뒤에도 종목명이 남아야 본 종목 기사로 본다.
- **`cafearticle` 검색엔 게시일 필드가 없다.** `ts_confidence='approx'`로 격리한다.
- **디시인사이드 주식갤러리(`stock_new1`)는 2017-01 이후 사실상 죽은 갤러리다.**
  커뮤니티 소스로 기대하지 말 것.
- **종토방은 대형주 백필이 구조적으로 불가능하다.** 하루 게시량이 2만 건인데
  게시판 페이지네이션이 **1,001페이지에서 소진**된다(실측 SK하이닉스 19,279건
  = 1.5일치). `--max-pages`를 올려도 소용없다 — 크롤러가 `stopped` 사유를
  `cutoff`/`page_cap`/`board_end`로 구분해 어느 레버가 유효한지 알려준다.
- **국내 국고채 금리는 무료 소스가 없다.** `KR10YT=RR` / `KR3YT=RR` / `KR1YT=RR`
  모두 404이고 한국은행 ECOS API는 키 발급이 필요하다. 그래서 채권은 금리가 아니라
  **ETF 가격**으로 담는다(KTB→148070 KOSEF 국고채10년, UST→IEF).
  UST만 금리로 남기면 두 채권이 다른 단위가 되는데, 시차 상관 표는 모든 엔티티를
  `close.pct_change()`로 수익률화하므로 금리 계열만 부호가 반대인 채(금리↑ = 채권가격↓)
  아무 표시 없이 실린다. **ETF는 순수 금리가 아니다** — 듀레이션·보수·추적오차가
  섞인다. [확인 필요]
- **알트코인 지수는 직접 만든 합성 계열이다.** 무료 소스가 없어 XRP·SOL·ADA·DOGE
  4종의 **일간 수익률을 동일가중 평균**해 100에서 체인으로 쌓는다. 가격을 평균하면
  안 된다 — SOL(13만원)과 DOGE(127원)를 평균하면 사실상 SOL 단독 지수가 된다.
  고가·저가는 합성 바스켓에 정의되지 않아(코인별 장중 고점 시각이 다르다) 시가·종가
  범위로만 채운다. **생존편향이 있다** — 오늘 상위인 코인으로 과거를 돌리는 것이다.
- **가격은 pykrx**(비공식 KRX 스크래핑). 키움 OpenAPI+는 32비트 파이썬 + 실계좌
  로그인이 매번 필요해 자동화에 부적합.
- **10년치 백필은 무료 소스로 불가능하다.** Google News RSS 아카이브가 얕다 —
  하루 창 기준 코스피/bitcoin이 2026년 100/78건, 2024년 6/50건, **2016년 3/3건**.
  한국어는 1년만 지나면 사실상 비어 있고 영어는 2022년이 실용 한계다. 빅카인즈는
  유료 전환, GDELT는 레이트리밋으로 검증 실패(`[확인 필요]`).
  → **백필이 아니라 전진 수집이 답이다.** 확보 가능한 만큼만 채우고 오늘부터 매일
  쌓는다. 가설 검증에는 10년이 필요 없다 — 코인은 2~3년에 사이클이 다 들어 있다.

## 로드맵

- [x] 스키마 v0.3 + 레거시 이관 / 인코딩 버그 수정
- [x] 신호 귀속일(`signal_date`) 분리 — 개장 전 컷오프
- [x] 백테스트 엔진 (4모드 × 비용 토글 + Buy&Hold)
- [x] FastAPI 서빙 + 일/주/월 차트 + 3선 분해
- [x] 매체 등급 매핑 (57% → 12.2%) + URL 기반 매체 복원
- [x] 채점기 (`irrelevant` + 반어법 few-shot + 분할 재시도) — 병렬 24건/초
- [x] 근사중복 + 도배 억제 + 반응 기반 표본
- [x] 온도지수에 글 수 반영 (신호 2배) + 적응형 백필 창
- [x] **엔티티 추상화** — 지수/코인/채권/원자재를 같은 스키마로
- [x] **다국어 수집** — 한/영 로케일 자동 분기, 별칭별 독립 질의
- [x] **4chan /biz/ 수집기** — 백필 불가 소스, 매일 실행
- [x] **스케줄러 등록** (매일 09:10 / 21:10 자동 수집)
- [x] **시장 간 비교 화면** (여론 vs 가격 시차 상관)
- [x] **수집/서빙 분리** — 집계 스냅샷 내보내기 (131.3 MB -> 6.2 MB)
- [x] **해외 매체 등급 분류** — wire/global/crypto 축 신설, 미분류 50.9% -> 17.0%
- [x] **채점 채널 정리 + 채점률 노출** — 호출 31% 절감, 미완 시장을 화면에서 격리
- [x] **005930 백필** — 13일치 -> 366일치(google_rss 22,269건), 채점률 12.7% -> 88.5%
- [x] **KTB · ALTCOIN 가격 확보** — 채권은 ETF로 단위 통일, 알트코인은 합성 지수
- [x] **검색어/별칭 분리(`queries_json`) + 005930 제외어** — 1년 실측 신규 8,629건 추가
- [x] **수집 미완결일을 화면에 노출** — 스냅샷 `date_coverage` -> 차트 하단 리본

### 코스피200 확장 시점으로 미룬 것

MinHash/TF-IDF 근사중복, 은어 사전, 로컬 모델 파인튜닝, PostgreSQL 전환
(트리거: 종목 30개 초과 **또는** `sentiment_daily` 300만 행 초과).

### 확인 필요 항목

- **개인 공매도 차입 실요율** — 공시가 없어 금감원 자료의 기관 평균 대여수수료(연 2.8%)를
  하한으로 사용 중. 실제는 이보다 높을 가능성이 크다
- `pykrx` 반환 가격의 수정주가 여부 (액면분할 미반영 시 백테스트 왜곡)
- 채권 ETF가 "금리 여론"의 대응물로 적절한가 (듀레이션·보수가 섞인다)
- 알트코인 바스켓 구성 4종이 "알트코인 여론"을 대표하는가 (생존편향 포함)
- 코스피200 과거 편입 종목 이력 (생존 편향)
- 실제 거래 수수료율 (현재 `fee_schedule`은 임시 기본값)

---

## 🎬 연계 프로젝트: 영상 자동 생성 스튜디오

10년치 감성 빅데이터를 활용하여 쇼츠/릴스 비교 영상을 자동으로 제작하는 데스크톱 GUI 프로그램:
- **프로젝트 위치**: [`D:\dev\trust-no-internet-video`](file:///D:/dev/trust-no-internet-video)
- **실행**: `run_gui.bat` 또는 `python run_gui.py`
- **주요 기능**: 10년 트렌드 비교 라인 차트 + 배경 감성 Bar 차트 + 무한공매도/수수료 시뮬레이션 + 3일 주기 일괄 생성기
