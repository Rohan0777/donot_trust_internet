...
숫자를 만지는 꼴이 된다). **[확인 필요]** 실제 요율이 미확보여서 현재 0(부과 없음)이고,
아래 표의 수치 전부가 이 비용이 빠진 값이다. 요율을 채우면 네 칸 모두 나빠진다 —
즉 현재 표는 비용 누락 쪽으로 관대한 상한선이다.
...
- **carry(차입·금융) 요율** — `fee_schedule`의 `borrow_bps_annual`/`financing_bps_annual`.
  입력 전까지 백테스트 수치는 이 비용이 빠진 값이며, 특히 무한공매도 칸이 가장 영향받는다
